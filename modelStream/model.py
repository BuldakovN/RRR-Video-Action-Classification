...
from utils import VideoIterator, ObjectsCache, pack_pathway


device = "cpu"

class2label ={0: 'abseiling',
 1: 'air drumming',
...
def load_slowfast():
    ...
    # Replace head
    # slowfast.blocks[6] = ResNetBasicHead(
    #     dropout=nn.Dropout(p=0.5, inplace=False),
    #     proj=nn.Linear(in_features=2304, out_features=24, bias=True),
    #     output_pool=nn.AdaptiveAvgPool3d(output_size=1),
    # )

    # slowfast.load_state_dict(torch.load("weights/slowfast_r50_last_layers_epoch2_0.674acc.pt", map_location="cpu")) # TODO
    slowfast.to(device)
    slowfast.eval();
    return slowfast

def convert_to_bytes(image: np.array):
...
def predict_stream(stream_path: str):
    # Initialize models
    yolo = YOLO("yolov8n.pt").to(device)
    slowfast = load_slowfast()
    objects_cache = ObjectsCache(transform)
    object_labels = {}

    video_iterator = VideoIterator(stream_path)
    for frame in video_iterator:
        # Detect and track
        result = yolo.track(frame, **yolo_kwargs)[0]
        if result.boxes.id is not None:
            ids = result.boxes.id.cpu().numpy()
            boxes = np.round(result.boxes.xyxy.cpu().numpy()).astype(int)
            objects_cache.add_objects(frame, ids, boxes)
        
        # Classify action
        ids, frames = objects_cache.get_tensors()
        for id, frames_tensor in zip(ids, frames):
            slow_pathway, fast_pathway = pack_pathway(frames_tensor)
            preds = slowfast([
                slow_pathway.to(device).unsqueeze(0),
                fast_pathway.to(device).unsqueeze(0),
            ])
            preds = torch.argmax(preds)
            object_labels[id] = class2label[int(preds)]
        ...
        for i in range(boxes.shape[0]):
            ...
            if result.boxes.id is not None and int(result.boxes.id[i]) in object_labels:
                frame = cv2.putText(
                    frame,
                    object_labels[int(result.boxes.id[i])],
                    (box[0], box[1]),
